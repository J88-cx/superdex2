...
namespace GestureSign.ControlPanel
{
    public partial class App : Application
    {
        ...
        {
            const string appUserModelId = "41908Transpy.GestureSign_f441wk0cxr8zc!GestureSign";
            const string registryPath = @"Software\Classes\ActivatableClasses\Package";
            ...
            using (var key = Registry.CurrentUser.OpenSubKey(registryPath))
            {
                ...
                {
                    using (var packageKey = key.OpenSubKey(subKey + @"\Server"))
                    {
                        if (packageKey == null) continue;

                        foreach (var serverSubKey in packageKey.GetSubKeyNames())
                        {
                            using (var serverKey = packageKey.OpenSubKey(serverSubKey))
                            {
                                var value = serverKey?.GetValue("AppUserModelId")?.ToString();
                                if (value == appUserModelId)
                                {
                                    try
                                    {
